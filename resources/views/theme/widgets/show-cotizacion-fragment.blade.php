            ...
            <div class="an-helper-block">
              ...
              <div class="an-input-group">
                ...
                <select class="an-form-control" id="clienteCodigo" name="clienteCodigo" >
                    <option selected value="{{ $cotizacion['Quotations'][0]->numCliente }}" > {{ $cotizacion['Quotations'][0]->numCliente . ' - ' . $cotizacion['Quotations'][0]->nombreCliente }}</option>
                </select>
              </div>
          ...
          <div class="col-md-2">
            <div class="an-input-group">
                ...
                <select id="formatoCoti" class="">
                    <option value="comp" @if($cotizacion['Quotations'][0]->formato == 'comp') {{ 'selected' }} @endif>Comparativo</option>
                    <option value="opc" @if($cotizacion['Quotations'][0]->formato == 'opc'){{ 'selected' }} @endif>Opciones</option>
                    <option value="siva" @if($cotizacion['Quotations'][0]->formato == 'siva'){{ 'selected' }} @endif>Sin Iva</option>
                </select>
                </div>
            </div>
          </div>
          <div class="col-md-2">
              <div class="an-input-group">
                  ...
                  <select id="langCoti" class="">
                      <option value="esp" @if($cotizacion['Quotations'][0]->lang == 'esp') {{ 'selected' }} @endif>Español</option>
                      <option value="eng" @if($cotizacion['Quotations'][0]->lang == 'eng') {{ 'selected' }} @endif>Inglés</option>
                  </select>
              </div>
          </div>
          <div class="col-md-2">
...
@foreach($cotizacion['ArtQuotation'] as $articulo)
    ...
    <div id="item-product[{{ $loop->iteration }}]" class="item-product row" data-type="update" data-product="{{ $articulo->id }}">
      <div class="row">  
        <div class="col-md-5">
          <div class="row">
            <div class="col-md-1">
            ...
            <div class="col-md-7">
              <div class="an-input-group">
                <textarea
                  ...
                  class="itemName an-form-control" 
                  data-toggle="tooltip" data-placement="top" title="{{ $articulo->nombreArt }}"
                  cols="30" rows="10">{{ $articulo->nombreArt }}</textarea>
                <!--<input type="text" 
                  id="itemName[{{ $loop->iteration }}]" 
                  name="itemName[{{ $loop->iteration }}]" 
    ...
    <div class="row">  
      ...
      <div class="col-md-7">
        <div class="row">
          ...
          <div class="col-md-2">
            <div class="an-input-group">
              <input type="number" id="itemCantidad[Element]" name="itemCantidad[Element]" class="itemCantidad an-form-control" value="0" min="1">
            </div>
          </div>
          <div class="col-md-2">
            <div class="an-input-group">
  ...
  $("#update").click(function(){
      ...
      $.ajax({
        ...
        success: function(result){
          ...
          $("#contenerdor-products").find(".item-id").each(function(key, item){
            $('#item-product\\[' + Number(key+1) +'\\]').attr("data-type", "update");
...
function calculoCotizacion(prod){
  ...
  itemPVenta = Number( parseFloat( Number(precioLista) * Number( 1 - descuento/100 ).toFixed(2) ) ).toFixed(2);
...
validarSAP();
  ...
  function validarCotizacionArticulos() {
    ...
    $("[id^='item-product']").each(function(index, element) {
        ...
        if ($(this).find("[name^='itemCantidad']").val() == "" || Number($(this).find("[name^='itemCantidad']").val()) == 0) {
