...
class PDFControllerPRUEBAS extends Controller
{
    ...
    public function pdfQuotation($numCotizacion,$auxprom="holadefault"){
        ...
        $Notes = Notes::find($Quotation->notasCotizacion) ? Notes::find($Quotation->notasCotizacion) : array ('condiciones' => 'sin condiciones');;
        ...
        foreach($ArtQuotation as $aq){
          ...
          $aux_iva = empty($aq->factor) ? 0.0 : $aux_pu * $aq->factor / 100 * $aq->cantidad;
          $aux_subtotal = empty($aq->cantidad) ? 0.0 : $aux_pu * intval($aq->cantidad);
          ...
          $rowArts[] = 
            [
              ...
              'subTotalLinea' => number_format($aq->subTotalLinea, 2, '.', ','),
       ...
       // $pdf = PDF::loadHTML(CustomBladeCompiler::render($template->data, $data))->setPaper('a4');
        ...
        $pdf = PDF::loadView('pdf.'.session('Company').'_quotationPRUEBAS', compact('data'))->setPaper('a4');
    ...
    public function pdfQuotationRenta($numCotizacion,$auxprom="holadefault"){
        
        $Quotation = Quotation::where('numCotizacion', $numCotizacion)->first();
        //Equipo propuesto
        $ArtQuotation = ArtRenta::where('numCotizacion', $numCotizacion)->get();
        //Condicones Comerciales
        $CondComerciales = RentaQuotation::where('numCotizacion', $numCotizacion)->get();
        $Vendedor = User::where('email', $Quotation->idVendedor)->first();   
        ...
        foreach($CondComerciales as $cc){
            
            $total_condicion_mxn += $cc->monto;
            ...
            [
                ...
                'punitario' => number_format($cc->punitario, '2', '.', ','),
                'monto' => number_format($cc->monto, '2', '.', ',')
            ];
        }
       ...
       // $pdf = PDF::loadHTML(CustomBladeCompiler::render($template->data, $data))->setPaper('a4');
        ...
        $pdf = PDF::loadView('pdf.'.session('Company').'_quotationrenta', compact('data'))->setPaper('a4');
