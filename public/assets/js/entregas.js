...
$(document).ready(function(){
	...
	$("#hojasETable").DataTable({
		"columnDefs": [
        	...
        	{"render" : function(data, type, row){
				        		return "<span class='numcotizacion'>" + data + "</span>";
				   		}, "targets" : 1 },
        	{"render" : function(data, type, row){
				        		return "<span class='razonCliente'>" + data + "</span>";
				   		}, "targets" : 2 },
        	{"render" : function(data, type, row){
				        		return "<span class='nombreCliente'>" + data + "</span>";
				   		}, "targets" : 3 },
            {"render" : function(data, type, row){
                            return "<span class='nombreCliente'>" + data + "</span>";
                        }, "targets" : 4 },
        	...
        	{"render" : function(data, type, row){
							return'<a target="_blank" href="editarHE/' + data + '" class="an-btn an-btn-icon small update-btn"><img class="edit" src="' + edit_img_route + '" height="30" width="30"/></a>';   
				   		}, "targets" : 6 },
        	{"render" : function(data, type, row){
                            return '<a target="_blank" href="pdf/'+ row[1] +'" data="' + data + '" class="open an-btn an-btn-icon small update-btn"><i class="icon-search"></i></a>';
				   		}, "targets" : 7 },
            {"render" : function(data, type, row){
...
function showUsers( data ){
	var TABLE = $("#hojasETable").DataTable();
	data.forEach(function(e){
		var array = new Array();
		array.push(e.id);
		array.push(e.numdelivery);
		array.push(e.razonCliente);
		array.push(e.nombreCliente);
        array.push(e.direccionCliente);
		array.push(e.created_at);
		array.push(e.numdelivery);
        array.push(e.id);
		array.push(e.id);
		TABLE.row.add( array );
		...
		$.ajax({
			...
			success : function( response ){
				if( response.status ){
					...
					var parents = $("#hojasETable span:contains('" + $("#id").val() +  "')").parents("tr");
	...
	$.ajax({
		...
		success : function( response ){
			if( response.status ){
				...
				var row = $("#hojasETable").DataTable().row( $(event.currentTarget).parents('tr') ).remove().draw();
			}
		}
	});
}
