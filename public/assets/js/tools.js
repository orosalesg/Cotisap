...
function setUpdateComponent(elem){
	...
	$("#action-btn").click( {id: id}, updateTool );
...
function addCreateListeners(){
	$("#action-btn").click( storeTool );
	$("#upload_file").click( uploadEvent );
	$("#input_file").on("change", setFileToUpload );
	$("#titTool").on("keyup", checkTitleInput );
	$("#brandTool").on("keyup", checkBrandInput );
	$("#descTool").on("keyup", checkDesc );
	$("#linkTool").on("keyup", checkLinkInput )
...
function loadToolInfo( id ){
	$("#idTool").val( id );
	$("#titTool").val($("#title-" + id).text());
	$("#brandTool").val($("#brand-" + id).text());
	$("#descTool").val($("#desc-" + id).text());
	$("#catTool").val($("#cat-" + id).text());
...
function storeTool(){
	...
	if( $("#link").prop("checked") ){
  	...
  	data.append('titulo', $("#titTool").val());
  	data.append('descripcion', $("#descTool").val());
  	data.append('marca', $("#brandTool").val());
...
function validateForm(){
	if( $("#link").prop("checked")){
		if( (checkTitleInput() & checkLinkInput()) & checkBrandInput() ){
			return true;
		}else{
			return false;
		}
	}else{
		if( (checkTitleInput() & checkFileInput()) & checkBrandInput() ){
			return true;
		}else{
			return false;
		}
...
function checkTitleInput(){
	var titToolInput = $("#titTool");
	var titToolIcon = $("#titToolIcon i");
	var val = titToolInput.val();
	if( isEmpty( val ) ){
		switchInputStatus( titToolInput, titToolIcon, "wrong", "El campo está vacío");
		console.log("title not ok");
		return false;
	}else{
		switchInputStatus( titToolInput, titToolIcon, "ok", "");
		return true;
	}	
}
function checkBrandInput(){
	var brandToolInput = $("#brandTool");
	var brandToolIcon = $("#brandToolIcon i");
	var val = brandToolInput.val();
	if( isEmpty( val ) ){
		switchInputStatus( brandToolInput, brandToolIcon, "wrong", "El campo está vacío");
		console.log("title not ok");
		return false;
	}else{
		switchInputStatus( brandToolInput, brandToolIcon, "ok", "");
		return true;
	}
}
...
function checkLinkInput(){
	var linkToolInput = $("#linkTool");
	var linkToolIcon = $("#linkToolIcon i");
	var val = linkToolInput.val();
	if( isEmpty( val ) ){

		console.log("link not ok");
		switchInputStatus( linkToolInput, linkToolIcon, "wrong", "El campo está vacío");
		return false;
	}else{
		switchInputStatus( linkToolInput, linkToolIcon, "ok", "");
		return true;
	}	
}
function checkDesc(){
...
function getAllTools(){
	$.get({
		...
		success : function( response ){
			...
			showTools( data );
			$("#data-loader").remove();
			$(".delete-btn").on("click", deleteTool );
			$(".update-btn").on("click", setUpdateComponent );
		}
	});
}
function showTools( data ){
	var TBODY = $("#tools-table tbody");
	var TABLE = $("#tools-table");
	for( var i = 0 ; i < data.length ; i++ ){
		var TR = $("<tr></tr>");
		var TD_ID = $("<td>" + data[ i ].id + "</td>");
		var TD_TITULO = $("<td class='text-center' id='title-" + data[ i ].id + "'>" + data[ i ].titulo + "</td>");
		...
		var TD_MARCA = $("<td id='brand-" + data[ i ].id + "' class='text-center'>" + data[ i ].marca + "</td>");
...
function updateTool( event ){
	...
	var data = new FormData();
  	data.append('titulo', $("#titTool").val());
  	data.append('descripcion', $("#descTool").val());
  	data.append('categoria', $("#catTool").val());
  	data.append('marca', $("#brandTool").val());
